                                    ...
                                    const std::string &model_path)
{
    ...
    const char *env = std::getenv("EVOKE_CHECKOUT_SIGNATURE");
...
{
    ModelInfo model;
#ifdef EVOKE_PACKAGED_MODEL_PATH
    ...
    if (json_string_default(manifest.get(), "runtime", "") !=
        ...
        json_string_default(manifest.get(), "runtime_abi", "") !=
            RUNTIME_ABI) {
        throw std::runtime_error("unsupported evoke model manifest");
    }
    ...
    void create_session(const Options &options)
    {
        ...
        for (const std::string &provider : providers) {
            ...
            try {
                ...
                std::cerr
                    << "evoke-runtime-server provider " << provider
                    << " unavailable: " << ex.what() << '\n';
                if (errors.tellp() > 0) {
                    errors << "; ";
                }
...
int main(int argc, char **argv)
{
    try {
        ...
        std::cerr
            << "evoke-runtime-server listening on "
        ...
        std::cerr << "evoke-runtime-server: " << error.what() << '\n';
        return 1;
    }
}
