CREATE EXTENSION evoke WITH SCHEMA public;

CREATE SCHEMA bm25_it;
SET search_path = bm25_it, public;

COPY (
    SELECT evoke_packaged_model_path_internal()
        LIKE '%/evoke/models/default'
) TO STDOUT;
...
COPY (
    ...
    WHERE n.nspname = 'public'
      AND p.proname IN (
        'evoke_hybrid_check_direction',
        'evoke_hybrid_check_fusion',
        'evoke_hybrid_check_normalizer',
        'evoke_hybrid_fuse_candidates_sql_reference',
        'evoke_fuse_results',
        'evoke_hits',
        'evoke_match_query_tokens',
        'evoke_query_hits',
        'evoke_score_query_tokens'
      )
) TO STDOUT;
...
CREATE FUNCTION bm25_it.hit_arrays(hits public.evoke_result_hit[])
...
AS $$
    ...
    FROM unnest(COALESCE($1, ARRAY[]::public.evoke_result_hit[])) AS h
$$;
...
AS $$
    ...
    FROM bm25_it.hit_arrays(ARRAY(
        SELECT h
        FROM public.evoke_query_ids($1, $2, $3, $4) AS h
    ))
$$;
...
AS $$
    ...
    FROM bm25_it.hit_arrays(ARRAY(
        SELECT h
        FROM public.evoke_query_tokens($1, $2, $3, $4) AS h
    ))
$$;
...
AS $$
    ...
    FROM bm25_it.hit_arrays(ARRAY(
        SELECT h
        FROM public.evoke_query($1, $2, $3, $4) AS h
    ))
$$;
...
AS $$
    ...
    FROM bm25_it.hit_arrays(ARRAY(
        SELECT h
        FROM public.evoke_fusion_query(
            $1,
            $2,
            $3,
            $4,
...
CREATE FUNCTION bm25_it.search_field_queries(
    field_queries public.evoke_result_fusion_field_query[],
    k int4 DEFAULT 10,
    candidate_k int4 DEFAULT NULL,
    weight_mask real[] DEFAULT NULL
)
...
AS $$
    ...
    FROM bm25_it.hit_arrays(ARRAY(
        SELECT h
        FROM public.evoke_fusion_query_fields($1, $2, $3, $4) AS h
    ))
$$;

CREATE FUNCTION bm25_it.search_weighted_queries(
    weighted_queries public.evoke_result_fusion_weighted_query[],
    k int4 DEFAULT 10,
    candidate_k int4 DEFAULT NULL,
    weight_mask real[] DEFAULT NULL
)
...
AS $$
    ...
    FROM bm25_it.hit_arrays(ARRAY(
        SELECT h
        FROM public.evoke_fusion_query_weighted($1, $2, $3, $4) AS h
    ))
$$;
...
AS $$
    ...
    FROM public.evoke_index_details($1) AS details
$$;
...
CREATE INDEX docs_tokens_bm25_idx
    ON docs_tokens USING evoke (tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);

COPY (
    SELECT public.evoke_index_details(
...
COPY (
    SELECT public.evoke_runtime_cache_clear() >= 0
) TO STDOUT;
...
CREATE INDEX docs_tokens_varchar_bm25_idx
    ON docs_tokens_varchar USING evoke (tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);
...
COPY (
    SELECT id
    FROM bm25_it.docs_tokens_varchar
    WHERE tokens @@@ public.evoke_prepared_query(
        'bm25_it.docs_tokens_varchar_bm25_idx'::regclass,
        'cat'
    )
    ORDER BY id
...
COPY (
    SELECT public.evoke_normalize_tokens(
...
CREATE INDEX docs_text_scalar_bm25_idx
    ON docs_text_scalar USING evoke (content)
    WITH (
        text_lowercase = true,
...
COPY (
    SELECT
        ...
        (q).fold_diacritics
    FROM (
        SELECT public.evoke_prepared_query(
            'bm25_it.docs_text_scalar_bm25_idx'::regclass,
            'CAFE BIRD'
        ) AS q
    ) prepared
...
COPY (
    ...
    WHERE public.evoke_match_query(
        content,
        'bm25_it.docs_text_scalar_bm25_idx'::regclass,
        'CAFE BIRD'
    )
...
COPY (
    SELECT id
    FROM bm25_it.docs_text_scalar
    ORDER BY public.evoke_score_query(
        content,
        'bm25_it.docs_text_scalar_bm25_idx'::regclass,
        'CAFE BIRD'
    ) DESC, id
...
COPY (
    SELECT id
    FROM bm25_it.docs_text_scalar
    WHERE content @@@ public.evoke_prepared_query(
        'bm25_it.docs_text_scalar_bm25_idx'::regclass,
        'cafe bird',
        true,
        NULL,
    ...
    ORDER BY content <=> public.evoke_order_tokens(
        'bm25_it.docs_text_scalar_bm25_idx'::regclass,
        'cafe bird',
        true,
        NULL,
...
CREATE INDEX docs_text_scalar_default_bm25_idx
    ON docs_text_scalar_default USING evoke (content);
...
BEGIN
    BEGIN
        EXECUTE
            ...
            || 'ON docs_text_scalar_default USING evoke (content) '
            || 'WITH (b = 1.01)';
        RAISE EXCEPTION 'evoke accepted b outside the BM25 domain';
    ...
    BEGIN
        EXECUTE
            ...
            || 'ON docs_text_scalar_default USING evoke (content) '
            || 'WITH (k1 = 3.5e38)';
        RAISE EXCEPTION 'evoke accepted k1 that cannot fit in real';
    EXCEPTION WHEN invalid_parameter_value THEN
        NULL;
    END;
    BEGIN
        EXECUTE
            ...
            || 'ON docs_text_scalar_default USING evoke (content) '
            || 'WITH (delta = 3.5e38)';
        RAISE EXCEPTION 'evoke accepted delta that cannot fit in real';
    EXCEPTION WHEN invalid_parameter_value THEN
        NULL;
    END;
    BEGIN
        EXECUTE
            ...
            || 'ON docs_text_scalar_default USING evoke (content) '
            || 'WITH (k1 = ''NaN'')';
        RAISE EXCEPTION 'evoke accepted non-finite k1';
    EXCEPTION WHEN invalid_parameter_value THEN
        NULL;
    END;
    BEGIN
        ...
        RAISE EXCEPTION 'evoke accepted retired churn reloption';
    EXCEPTION WHEN invalid_parameter_value THEN
        NULL;
    END;
    BEGIN
        ...
        RAISE EXCEPTION 'evoke accepted retired overlay-record reloption';
    EXCEPTION WHEN invalid_parameter_value THEN
        NULL;
    END;
    BEGIN
        ...
        RAISE EXCEPTION 'evoke accepted retired overlay-byte reloption';
    EXCEPTION WHEN invalid_parameter_value THEN
        NULL;
    END;
END;
...
COPY (
    SELECT public.evoke_order_tokens(
        'bm25_it.docs_text_scalar_default_bm25_idx'::regclass,
        'CAT'
    )::text
) TO STDOUT;
...
CREATE INDEX docs_text_pipeline_bm25_idx
    ON docs_text_pipeline USING evoke (content)
    WITH (
        text_lowercase = true,
...
CREATE INDEX docs_varchar_scalar_bm25_idx
    ON docs_varchar_scalar USING evoke (content)
    WITH (
        text_lowercase = true,
        text_fold_diacritics = true
    );
...
SELECT public.evoke_match_query(
    'Café bird',
    'bm25_it.docs_text_scalar_bm25_idx'::regclass,
    'cafe bird',
    true,
...
SELECT public.evoke_score_query(
    'Café bird',
    'bm25_it.docs_text_scalar_bm25_idx'::regclass,
    'cafe bird',
    true,
...
SELECT public.evoke_match_prepared_query(
    'Café bird'::varchar,
    public.evoke_prepared_query(
        'bm25_it.docs_varchar_scalar_bm25_idx'::regclass,
        'cafe bird',
        true,
        NULL::text[],
...
SELECT public.evoke_score_prepared_query(
    'Café bird'::varchar,
    public.evoke_prepared_query(
        'bm25_it.docs_varchar_scalar_bm25_idx'::regclass,
        'cafe bird',
...
COPY (
    WITH qs AS (
        SELECT public.evoke_fusion_weighted_queries(
            ARRAY[
                'bm25_it.docs_tokens_bm25_idx'::regclass,
                'bm25_it.docs_tokens_bm25_idx'::regclass
            ],
...
COPY (
    WITH qs AS (
        SELECT public.evoke_fusion_field_queries(
...
CREATE INDEX docs_tokens_multi_fused_bm25_idx
    ON docs_tokens_multi USING evoke (title_tokens, body_tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);
...
CREATE INDEX docs_tokens_multi_combined_bm25_idx
    ON docs_tokens_multi USING evoke (combined_tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);
...
COPY (
    ...
    FROM public.evoke_field_aware_query_tokens(
...
COPY (
    SELECT d.id, round(h.score::numeric, 6)::text
    FROM public.evoke_field_aware_query(
        'bm25_it.docs_tokens_multi_field_bm25_idx'::regclass,
        'bird',
        ARRAY['title_tokens', 'body_tokens'],
        ARRAY[3.0, 1.0]::real[],
...
CREATE INDEX docs_tokens_multi_field_delta_bm25_idx
    ON docs_tokens_multi_field_delta USING evoke (title_tokens, body_tokens)
    WITH (
        method = 'lucene',
        idf_method = 'lucene',
        k1 = 1.5,
...
CREATE INDEX docs_tokens_multi_eventual_field_delta_bm25_idx
    ...
    USING evoke (title_tokens, body_tokens)
    WITH (
...
COPY (
    ...
    SELECT
        (
            ...
            public.evoke_index_shared_preload_resident(
...
CREATE INDEX docs_tokens_multi_varchar_fused_bm25_idx
    ON docs_tokens_multi_varchar USING evoke (title_tokens, body_tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);

CREATE INDEX docs_tokens_multi_varchar_combined_bm25_idx
    ON docs_tokens_multi_varchar USING evoke (combined_tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);
...
COPY (
    SELECT
        ...
        h.source_names::text,
        h.ranks::text
    FROM public.evoke_hybrid_fuse_candidates(
        ARRAY[
            public.evoke_hybrid_bm25_candidate(
                'title',
                '(0,1)'::tid,
                4.0,
                1,
            ...
            public.evoke_hybrid_vector_candidate(
                'embedding',
                '(0,1)'::tid,
                0.4,
                2,
        ...
        ]::public.evoke_result_hybrid_candidate[],
        3,
        'rrf',
        60.0
    ) AS h
...
COPY (
    SELECT count(*)::text
    FROM public.evoke_hybrid_fuse_candidates(
        ARRAY[]::public.evoke_result_hybrid_candidate[],
        10,
        'rrf'
    )
) TO STDOUT;
...
COPY (
    WITH candidates AS (
        ...
        FROM public.evoke_hybrid_bm25_candidates(
            'tokens',
            'bm25_it.docs_tokens_bm25_idx'::regclass,
            'bird',
            2.0,
    ...
    SELECT
        ...
        h.ranks::text
    FROM candidates
    CROSS JOIN LATERAL public.evoke_hybrid_fuse_candidates(
        candidates.arr,
        3,
        'rrf'
    ) AS h
...
BEGIN
    ...
    WHERE title_tokens @@@ public.evoke_prepared_query(
        'bm25_it.docs_tokens_multi_fused_bm25_idx'::regclass,
        'cat'
    );
EXCEPTION
...
CREATE INDEX docs_text_multi_scalar_fused_bm25_idx
    ON docs_text_multi_scalar USING evoke (title_text, body_text)
    WITH (
        method = 'lucene',
        idf_method = 'lucene',
        k1 = 1.5,
...
CREATE INDEX docs_text_multi_scalar_combined_bm25_idx
    ON docs_text_multi_scalar USING evoke (combined_text)
    WITH (
        method = 'lucene',
        idf_method = 'lucene',
        k1 = 1.5,
...
CREATE INDEX docs_varchar_multi_scalar_fused_bm25_idx
    ON docs_varchar_multi_scalar USING evoke (title_text, body_text)
    WITH (
        method = 'lucene',
        idf_method = 'lucene',
        k1 = 1.5,
...
CREATE INDEX docs_varchar_multi_scalar_combined_bm25_idx
    ON docs_varchar_multi_scalar USING evoke (combined_text)
    WITH (
        method = 'lucene',
        idf_method = 'lucene',
        k1 = 1.5,
...
COPY (
    ...
    FROM (
        SELECT bm25_it.search_field_queries(
            ARRAY[
                public.evoke_fusion_field_query(
                    'title',
                    'bm25_it.docs_tokens_bm25_idx'::regclass,
                    'bird',
                    2.0
            ...
            ]::public.evoke_result_fusion_field_query[],
            3,
            3,
            NULL
        ) AS r
...
COPY (
    ...
    FROM (
        SELECT bm25_it.search_weighted_queries(
            ARRAY[
                public.evoke_fusion_weighted_query(
                    'bm25_it.docs_tokens_bm25_idx'::regclass,
                    'bird',
                    2.0
                ),
            ...
            ]::public.evoke_result_fusion_weighted_query[],
            3,
            3,
            NULL
        ) AS r
...
COPY (
    ...
    WHERE public.evoke_match_prepared_query(
        tokens,
        public.evoke_prepared_query(
            'bm25_it.docs_tokens_bm25_idx'::regclass,
            'CAT and BIRD',
...
COPY (
    SELECT id, public.evoke_score_query(
        tokens,
        'bm25_it.docs_tokens_bm25_idx'::regclass,
        'CAT and BIRD',
        true,
...
COPY (
    SELECT id, public.evoke_score_prepared_query(
        tokens,
        public.evoke_prepared_query(
            'bm25_it.docs_tokens_bm25_idx'::regclass,
            'CAT and BIRD',
...
COPY (
    ...
    ORDER BY tokens <=> public.evoke_order_tokens(
        public.evoke_prepared_query(
            'bm25_it.docs_tokens_bm25_idx'::regclass,
            'bird cat missing'
        )
...
CREATE INDEX docs_tokens_candidate_bm25_idx
    ON docs_tokens_candidate USING evoke (tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);
...
CREATE INDEX docs_tokens_candidate_mid_bm25_idx
    ON docs_tokens_candidate_mid USING evoke (tokens)
    WITH (
        method = 'lucene',
        idf_method = 'lucene',
        k1 = 1.5,
...
SELECT public.evoke_tokenize_text(
    'Cat, and dog!',
    true,
    ARRAY['and']::text[]
);
...
CREATE INDEX docs_tokens_stem_bm25_idx
    ON docs_tokens_stem USING evoke (tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);
...
CREATE INDEX docs_tokens_fold_bm25_idx
    ON docs_tokens_fold USING evoke (tokens)
    WITH (method = 'lucene', idf_method = 'lucene', k1 = 1.5, b = 0.75, delta = 0.5);
...
CREATE INDEX docs_ids_bm25_idx
    ON docs_ids USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
COPY (
    SELECT public.evoke_index_policy_recommend(
        'bm25_it.docs_ids_bm25_idx'::regclass,
        'query_first'
    )
) TO STDOUT;
...
SELECT substring(
    public.evoke_index_runtime_state(
        'bm25_it.docs_ids_bm25_idx'::regclass
    ) FROM 'cache_epoch=([0-9]+)'
)::bigint AS epoch;

COPY (
    SELECT public.evoke_index_refresh(
        'bm25_it.docs_ids_bm25_idx'::regclass
    )
) TO STDOUT;
...
CREATE INDEX docs_ids_auto_bm25_idx
    ON docs_ids_auto USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
CREATE INDEX docs_partitioned_bm25_idx
    ON docs_partitioned USING evoke (body);
...
COPY (
    ...
    FROM (
        SELECT public.evoke_index_status(
...
COPY (
    SELECT current_setting('evoke.workspace_cache_bytes') IS NOT NULL
) TO STDOUT;
COPY (
    SELECT current_setting('evoke.workspace_idle_timeout') IS NOT NULL
) TO STDOUT;

SET evoke.workspace_cache_bytes = '0';
SET evoke.workspace_idle_timeout = '0ms';
...
RESET evoke.workspace_cache_bytes;
RESET evoke.workspace_idle_timeout;

SET evoke.workspace_cache_bytes = '-1';
SET evoke.workspace_idle_timeout = '-1';
RESET evoke.workspace_cache_bytes;
RESET evoke.workspace_idle_timeout;
...
BEGIN
    PERFORM *
    FROM public.evoke_index_maintain_due(1);
END
$$;
COPY (
...
CREATE INDEX docs_text_eager_unchanged_bm25_idx
    ON docs_text_eager_unchanged USING evoke (tokens);
...
CREATE INDEX docs_expr_eager_commit_bm25_idx
    ON docs_expr_eager_commit USING evoke (
        evoke_tokenize_text(
            COALESCE(title, ''::text),
            true,
            NULL::text[],
...
ON docs_ids_ties USING evoke (token_ids)
WITH (
    method = 'lucene',
    idf_method = 'lucene',
    k1 = 1.5,
...
ON docs_ids_sparse_paths USING evoke (token_ids)
WITH (
    method = 'lucene',
    idf_method = 'lucene',
    k1 = 1.5,
...
ON docs_ids_zero_idf USING evoke (token_ids)
WITH (
    method = 'robertson',
...
ON docs_ids_mvcc_sparse_expand USING evoke (token_ids)
WITH (
    method = 'lucene',
    idf_method = 'lucene',
    k1 = 1.5,
...
CREATE INDEX docs_ids_auto_txn_bm25_idx
    ON docs_ids_auto_txn USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
CREATE INDEX docs_ids_auto_query_bm25_idx
    ON docs_ids_auto_query USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
CREATE INDEX docs_ids_auto_threshold_bm25_idx
    ON docs_ids_auto_threshold USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
CREATE INDEX docs_text_auto_threshold_bm25_idx
    ON docs_text_auto_threshold USING evoke (tokens)
    WITH (
        method = 'lucene',
        idf_method = 'lucene',
        k1 = 1.5,
...
CREATE INDEX docs_ids_auto_threshold_update_bm25_idx
    ON docs_ids_auto_threshold_update USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
CREATE INDEX docs_ids_auto_threshold_bytes_bm25_idx
    ON docs_ids_auto_threshold_bytes USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
CREATE INDEX docs_ids_auto_threshold_ratio_bm25_idx
    ON docs_ids_auto_threshold_ratio USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
...
CREATE INDEX docs_ids_eventual_policy_bm25_idx
    ON docs_ids_eventual_policy USING evoke (token_ids)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_default_policy_bm25_idx
    ON docs_default_policy USING evoke (tokens);
...
CREATE INDEX docs_eventual_default_policy_bm25_idx
    ON docs_eventual_default_policy USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_manual_payload_policy_bm25_idx
    ON docs_manual_payload_policy USING evoke (tokens)
    WITH (
        consistency = 'manual'
    );
...
COPY (
    WITH state AS (
        SELECT *
        FROM public.evoke_index_details(
...
CREATE INDEX docs_eventual_large_delta_policy_bm25_idx
    ON docs_eventual_large_delta_policy USING evoke (body)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_realtime_large_delta_policy_bm25_idx
    ON docs_realtime_large_delta_policy USING evoke (body)
    WITH (
...
DO $$
BEGIN
    PERFORM public.evoke_index_maintain(
        'bm25_it.docs_realtime_large_delta_policy_bm25_idx'::regclass
    );
END;
$$;
...
CREATE INDEX docs_manual_large_delta_policy_bm25_idx
    ON docs_manual_large_delta_policy USING evoke (body)
    WITH (
        consistency = 'manual'
    );
...
CREATE INDEX docs_text_eventual_budget_bm25_idx
    ON docs_text_eventual_budget USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_text_exact_budget_bm25_idx
    ON docs_text_exact_budget USING evoke (tokens);
...
CREATE INDEX docs_text_exact_delete_budget_bm25_idx
    ON docs_text_exact_delete_budget USING evoke (tokens);
...
CREATE INDEX docs_invalid_manual_threshold_bm25_idx
    ON docs_invalid_policy_options USING evoke (tokens)
    WITH (
        consistency = 'manual',
...
CREATE INDEX docs_eventual_overlay_budget_bm25_idx
    ON docs_eventual_overlay_budget USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
COPY (
    ...
    action AS MATERIALIZED (
        SELECT public.evoke_index_try_maintain(
            'bm25_it.docs_eventual_overlay_budget_bm25_idx'::regclass
        ) AS result
    ),
    after_state AS (
...
CREATE INDEX docs_eventual_delta_bytes_due_bm25_idx
    ON docs_eventual_delta_bytes_due USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_eventual_delta_bytes_oversized_bm25_idx
    ON docs_eventual_delta_bytes_oversized USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_eventual_delta_bytes_delete_bm25_idx
    ON docs_eventual_delta_bytes_delete USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_eventual_writer_lock_bm25_idx
    ON docs_eventual_writer_lock USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_text_eventual_unchanged_bm25_idx
    ON docs_text_eventual_unchanged USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_text_eventual_online_bm25_idx
    ON docs_text_eventual_online USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_due_priority_small_bm25_idx
    ON docs_due_priority_small USING evoke (tokens)
    WITH (consistency = 'eventual');

DO $$
BEGIN
    IF NOT public.evoke_index_try_maintenance_lock(
...
CREATE INDEX docs_due_priority_big_bm25_idx
    ON docs_due_priority_big USING evoke (tokens)
    WITH (consistency = 'eventual');
...
COPY (
    WITH small_state AS (
        ...
        FROM public.evoke_index_policy_recommend(
            'bm25_it.docs_due_priority_small_bm25_idx'::regclass,
            'balanced'
        )
    ),
...
DO $$
BEGIN
    PERFORM public.evoke_index_maintenance_unlock(
        'bm25_it.docs_due_priority_small_bm25_idx'::regclass
    );
    PERFORM public.evoke_index_maintenance_unlock(
        'bm25_it.docs_due_priority_big_bm25_idx'::regclass
...
CREATE INDEX docs_text_segment_reuse_bm25_idx
    ON docs_text_segment_reuse USING evoke (tokens)
    WITH (
        consistency = 'eventual'
    );
...
CREATE INDEX docs_ids_auto_threshold_delete_bm25_idx
    ON docs_ids_auto_threshold_delete USING evoke (token_ids)
    WITH (
        method = 'bm25+',
        idf_method = 'bm25+',
        k1 = 1.5,
