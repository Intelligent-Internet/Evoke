...
DEFAULT_OUTPUT = (
    Path(tempfile.gettempdir()) / 'evoke_product_path_benchmark.json'
)
DEFAULT_QUERIES = [
...
def configure_accelerator_route(
    ...
    cur.execute(
        ...
        "'evoke.test_disable_semantic_accelerator', %s, false)",
    ...
    cur.execute(
        "SELECT pg_catalog.set_config("
        "'evoke.test_semantic_accelerator_summary_multiplier', %s, false)",
    ...
    if heap_factor is not None:
        ...
        cur.execute(
            "SELECT pg_catalog.set_config("
            "'evoke.test_semantic_accelerator_candidate_multiplier', "
            "%s, false)",
...
def configure_postgres(data_dir: Path, args: argparse.Namespace) -> None:
    with (data_dir / 'postgresql.conf').open('a', encoding='utf-8') as handle:
        ...
        handle.write(
            f"evoke.shared_runtime_size = '{args.shared_cache_mb}MB'\n"
        )
        handle.write(
            'evoke.onnxruntime_intra_op_threads = '
            f'{args.onnxruntime_intra_op_threads}\n'
        )
        handle.write('evoke.preload_timer_interval_ms = 1000\n')
        handle.write('evoke.maintenance_timer_interval_ms = 60000\n')
...
def setup_bm25(
    ...
    results['body_index_seconds'] = timed(cur, 'create body index', """
        CREATE INDEX docs_body_evoke_idx
        ON bench.docs USING evoke (body)
        WITH (method = 'lucene', idf_method = 'lucene', consistency = 'manual')
    """)
    results['field_index_seconds'] = timed(cur, 'create field-aware index', """
        CREATE INDEX docs_title_body_field_evoke_idx
        ON bench.docs USING evoke (title, body)
        WITH (
            method = 'lucene',
            idf_method = 'lucene',
            consistency = 'manual',
    ...
    cur.execute("""
        ...
        WHERE relname IN (
            'docs',
            'docs_body_evoke_idx',
            'docs_title_body_field_evoke_idx'
        )
        ORDER BY relname
    """)
    ...
    body_sql = """
        SELECT d.id, h.score
        FROM evoke_query(
            'bench.docs_body_evoke_idx'::regclass,
            %s,
            20
        ) AS h
    ...
    field_sql = """
        ...
        FROM evoke_field_aware_query(
            'bench.docs_title_body_field_evoke_idx'::regclass,
            %s,
...
def setup_semantic(
    ...
    cur.execute("""
        SELECT evoke_index_status('bench.docs_body_semantic_idx'::regclass)
    """)
    results['status'] = cur.fetchone()[0]
    cur.execute("""
        SELECT evoke_index_audit('bench.docs_body_semantic_idx'::regclass)
    """)
...
def main() -> int:
    ...
    root = Path(tempfile.mkdtemp(prefix='evoke_product_pg_', dir=args.work_dir))
    data_dir = root / 'data'
    ...
    try:
        ...
        with psycopg.connect(dsn, autocommit=True) as conn:
            with conn.cursor() as cur:
                timed(cur, 'create extension', 'CREATE EXTENSION evoke')
                cur.execute('SELECT evoke_onnxruntime_build_info()')
                ...
                if (
                    ...
                    raise RuntimeError(
                        ...
                        'ONNX-enabled evoke build; rebuild with '
                        'EVOKE_ENABLE_ONNXRUNTIME=1 or pass --skip-semantic for '
