...
- `shared_preload_libraries = 'evoke'` starts the supervisor and runtime pool.
- A positive `evoke.shared_runtime_size` provides bounded queues,
...
`evoke.runtime_reserve_query_lane = on` and at least two healthy workers,
...
`evoke.onnxruntime_session_cache_size = 0` releases a session after each request.
...
With multiple workers and `evoke.onnxruntime_intra_op_threads = 0`, Evoke
...
evoke.runtime_liveness_timeout = '5min'
evoke.onnxruntime_session_cache_size = 1
evoke.onnxruntime_intra_op_threads = 0
```

The official package installs the milestone checkout in PostgreSQL's shared
data directory. `evoke.sae_model_path` remains an optional server-wide override;
...
application database. Set `evoke.control_database` only when `postgres` is
unavailable or a different stable, connectable database is preferred.

`evoke.runtime_liveness_timeout` also bounds an already-submitted remote
...
`evoke_query(...)` opens only a root with the same signature. A runtime-contract
