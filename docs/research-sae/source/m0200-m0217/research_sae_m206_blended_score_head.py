#!/usr/bin/env python3
"""M206 blended-score mini-vector head.

M204/M205 showed that matching dense score shape is not enough for the
head0/window256/weight1.5 operator.  This trainer optimizes the blended final
score directly: baseline_score + dense_weight * minmax(mini_vector_score).
...
SCHEMA = 'm206_blended_score_head_v1'
PRIMARY_METRICS = ('map@100', 'recall@100')


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Train mini-vector heads against blended final scores.',
    )
    ...
    *,
    ...
    for epoch in range(epochs):
        ...
        for bundle_index, row in samples:
            ...
            residual_scores = doc_values @ query_values
            baseline = torch.from_numpy(
                baseline_windows[bundle_index][row],
            ).to(device)
            teacher = torch.from_numpy(bundle.teachers[row]).to(device)
            target_final = baseline + dense_weight * torch_minmax(teacher)
            predicted_final = (
                baseline + dense_weight * torch_minmax(residual_scores)
            )
            target = normalize(target_final)
            prediction = normalize(predicted_final)
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0:
            base.log(
                f'm206 dim={dimension} epoch={epoch + 1}/{epochs} '
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    payload = {
        ...
        'training_scope': 'shared projection head against blended final score',
        'policy': {
