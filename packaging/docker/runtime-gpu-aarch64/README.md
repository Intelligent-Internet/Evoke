...
`evoke-runtime-server` process. It does not start PostgreSQL, does not install
the evoke extension SQL, and does not own relation storage. The coordinator
...
- the default model checkout named build context into
  `/usr/local/share/evoke/models/default`;
- a Linux aarch64 CUDA 13 / SM121 ONNX Runtime 1.29 C artifact and CUDA runtime
  libraries from the `evoke_gpu_rootfs` named build context;
...
The `evoke_gpu_rootfs` build context must provide GPU ONNX Runtime and CUDA
...
scripts/install_onnxruntime_c.sh \
    ...
    --prefix /tmp/evoke-gpu-rootfs/usr/local
```
...
docker build \
    ...
    --build-context evoke_gpu_rootfs=/tmp/evoke-gpu-rootfs \
...
the runtime libraries in `evoke_gpu_rootfs`.
...
docker run -d --name evoke-runtime-gpu --gpus all --ipc=host \
    -p 18042:8042 \
    -e EVOKE_CHECKOUT_SIGNATURE="$CHECKOUT_SIGNATURE" \
    -e EVOKE_HTTP_WORKER_COUNT=8 \
    -e EVOKE_HTTP_QUEUE_DEPTH=48 \
    -e EVOKE_MAX_BATCH_SIZE=128 \
    -e EVOKE_MAX_DELAY_MS=5 \
    -e EVOKE_MAX_REQUEST_BYTES=67108864 \
    evoke-runtime-gpu:direct-cuda13
```

`EVOKE_CHECKOUT_SIGNATURE` is required. The direct service intentionally has no
...
`EVOKE_MODEL_PATH` defaults to `/usr/local/share/evoke/models/default`.

`EVOKE_MAX_BATCH_SIZE` defaults to 128 and can be raised up to 512. It is a
...
`EVOKE_HTTP_QUEUE_DEPTH` controls runtime-server backpressure. When omitted,
the server derives a bounded queue from `EVOKE_HTTP_WORKER_COUNT`; when the
...
`EVOKE_INTRA_OP_THREADS` is optional. Leave it unset for the default ONNX
